...
- give every command surface a default timeout
- raise TS/vitest strictness to Python-tree parity
- per-ticket coverage follow-ups — config-hardened git, drift guards, ticket UX
- per-ticket coverage attribution, #/tickets page, and tickets.json export


### Documentation

- drop a .gcov artifact claim the pipeline never produced


### Fixed

- make the tier-1 perf guards deterministic — count work, not time
- give the series chips an accessible name, un-mute the warning
- un-hand-edit the vendored slideout scrim, move z-50 to callers
- key column claimed a focused context on every page
