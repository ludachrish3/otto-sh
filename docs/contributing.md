...
`make coverage` is the same shape: `coverage-python` (full pytest, 95.5 floor over lines and branches)
...
| otto_kgcov toolchain proofs (opt-in, dev VM only; `make release` runs it) | `make kgcov` (`KGCOV_TOOLCHAINS=gcc-9,…,clang`, `KGCOV_CROSS_KDIR=<prepared kernel source tree>`); ends by folding the run into `schemas/kgcov_matrix.json` (the {ref}`compatibility matrix <kgcov-matrix>`), which `make release-kgcov-matrix` re-measures and commits | test1/test2, plus gcc 9–14, clang 18 and the source tree on the dev VM |
