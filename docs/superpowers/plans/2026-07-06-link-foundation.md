...
7. **Dynamic wins by REPLACE, not field-merge, in `all_links`** — on a route-id collision the dynamic link *replaces* the declared one in the merged map, so the declared `name` and the resolved endpoint `ip`s are dropped. Field-level enrichment (carry the declared `name`/ips onto the observed dynamic link so the GUI shows both the friendly handle and the addresses) is **#2's responsibility**, not the foundation's.
8. **`otto link add` (#2) MUST resolve endpoints through the same interface-assumption rules as `_resolve_endpoint`** (omitted interface on a single-interface host → that interface; management ip on a zero-interface host) *before* computing the link id. Otherwise a dynamically-added link over a declared route computes a different id (e.g. `interface=None` vs the assumed `"eth1"`) and won't reconcile to its declared route id.
9. **Declared links cannot name the built-in `local` host, and docker placeholders surface as `local↔container` edges — both are #6 topology concerns.** `local` is injected in `configmodule.load_lab` *after* `JsonFileLabRepository.load_lab` has already resolved declared links, so a link endpoint referencing `local` would fail to resolve at load; docker placeholder hosts currently appear as `local↔container` implicit edges in `static_links()`. Deciding how the renderer treats these belongs to #6.
10. **Cross-lab link error-containment (this fix wave):** unrelated-lab links — those with **both** endpoints outside the requested lab — are **skipped** during `resolve_declared_links` (relevance read from the raw entry's endpoint host ids before strict `LinkSpec` validation), symmetric with the cross-lab **host-record** containment `JsonFileLabRepository.load_lab` already applies. A link **touching** the requested lab (≥1 endpoint in the loaded host set) still resolves and still **fails loud** with a `LabRepositoryError` naming the entry's original file index + source. This freezes the decision that a typo in an unrelated (shared-dir) lab's `links` section cannot break a sibling lab's load, while in-lab typos remain fatal.

## Foundation follow-ups (noted, deliberately NOT fixed in #1)

Small gaps this fix wave chose to leave for the owning sub-project — recorded so they aren't lost:

- **(T5)** `implicit_links` gives the built-in `local` endpoint `ip=""` in production (it is only exercised via fakes in unit tests). A #6 renderer concern — the base topology needs a real `local` address or an explicit "you are here" node treatment.
- **(T8)** The per-spec call site of `_inject_interface_shorthand` (the standalone `unix-host` / `embedded-host` schemas) has no dedicated test; it is only covered transitively.
- **(T9)** `docs/guide/lab-config.md`'s Links example references `tomato_seed`, which is not defined in that page's own Example section (doc-internal consistency, not a code defect).
