...
An **options class** declares CLI flags once and shares them across
instructions and suites. Both `otto run` and `otto test` expand them into
flat CLI options.
...
See {doc}`index` and {doc}`../guide/cli/test/index` for the full instruction and suite guides, and
