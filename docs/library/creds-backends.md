...
| `lookup(key)` | A `list` of {class}`~otto.models.host.CredSpec` for the inventory key — **`[]` when the store holds none for it**, never `None`, never a raise. Raise {class}`~otto.creds.errors.CredsError` only when the store cannot answer at all. Entries must be unique by login plus `protocols` scope within one key ({ref}`cred-protocols`); the list must be equal on a second call. |
...
- **Return whole entries, not passwords.** An entry may carry a route
  ...
  store that knows only logins and passwords is the common case. An entry may
  carry `protocols` too, when your source of record knows a login is for one
  service only; the field passes through the merge as part of the entry's
  identity.
