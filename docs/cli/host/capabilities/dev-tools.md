...
|------|----------|----------|--------------|
| **Dev tool** | in `settings.toml` (`[[dev_tools]]`, {doc}`../../../guide/configuration/declared-products-tools`) or in code, via a provider | the repo that declared/registered it | `install-tools` (on by default) |
