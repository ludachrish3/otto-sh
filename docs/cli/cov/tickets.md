...
and in `tickets.json`: **`(uncommitted)`** for working-tree lines that haven't been
...
tickets page states this under its table: rows overlap and do not sum to
the stats card above them, which counts each attributed line once
regardless of how many tickets claim it. The same rule applies to
...
`[coverage.overrides]` key), neither feature runs — no store change and no
report change.

The file is a hand-edited, commented TOML record, reviewed like any other
change. Default location is `.otto/coverage-overrides.toml`, next to
`settings.toml`, versioned with the SUT repo. Point at a different path
with:
...
Every rule below fails the load with a named error:
...
- Unknown keys in any entry are errors.
- **An override file requires `[coverage.tickets]` to be configured.**
  A present override file without that block is a config error, not a
  silent no-op.
...
runs, so every consumer (tickets page, file-page gutter, ticket context,
`tickets.json`, and asserted entries keyed on the corrected ticket) sees
the corrected ids.
...
Attribution is *live*, so the `as_of` bound is what keeps a later commit
under an old ticket from inheriting the entry's asserted coverage. A
commit-keyed entry needs no bound; the sha it names already is one.

**Aging is by content, not authorship.** A line that's later
rewritten migrates, by ordinary attribution supersession, to the newer
commit — and drops out of the entry's line set automatically. Whitespace-only
...
- The `⋮` menu's **"Hide asserted coverage"** toggle recomputes every
  ...
  tickets page's own aggregate/row stats show a dash (`—`, no data) under
  the toggle instead of a recomputed number.
...
file. The message distinguishes the two causes:
...
summary with a stable, versioned format, for CI dashboards, ticket-coverage
bots and ad-hoc scripts. (`store.json`, by contrast, is an internal
artifact whose shape may change on any `otto` release.) An example:
...
configured — that is, whenever `.otto/coverage-overrides.toml`
...
- **`format` is its own integer, versioned independently of
  ...
  **`format` 2** (up from 1) added each ticket's `asserted` map and the
  ...
  shape) of each ticket's `commits` array. v1 (shipped in v0.8.1) listed
  only the commits that currently own an attributed line for that ticket;
  v2 lists **every** commit the first-parent walk visited that named the
  ticket, including a commit whose lines have since been rewritten or
  superseded. Membership is unchanged: a ticket appears in `tickets` only
  when it owns at least one coverable line.
- **Output is deterministic apart from the `generated` timestamp**:
  ...
  data at the same `generated` stamp produce byte-identical files, so a
  diff between two regenerations (the timestamp aside) is exactly the
  coverage delta.
...
- **`missing` ranges are inclusive `[start, end]` pairs** — `[142, 142]`
  for a single line — the same grouping the tickets page renders.
