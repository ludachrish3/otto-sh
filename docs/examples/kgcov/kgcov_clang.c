...
/*
 * Vendored from Linux v6.8 kernel/gcov/clang.c for otto_kgcov. Changed from
 * the original: the include name; llvm_gcov_init() serialises its writeout
 * window under a lock of its own and hands the finished gcov_info to
...
/*
 * current_info's window spans the writeout callback's five calls. The
 * registering thread runs constructors under otto_kgcov's own lock, but a
 * CONFIG_CONSTRUCTORS kernel's loader runs a module's constructors during
 * load, on any CPU, while a walk may be in flight elsewhere — so the window
 * is serialised here. It is released before kgcov_ctor_info(), so the
 * library's lock is never taken under this one; the other order is the
 ...
 * kernel/gcov/base.c, not kernel/gcov/clang.c. Vendored verbatim from Linux
 * v6.8 kernel/gcov/base.c (the same pair kgcov_gcc.c carries, for the same
...
/* ---- otto_kgcov's backend hooks ---------------------------------------- */

/*
 * clang writes the gcc 4.8 file format whatever its own version (measured
 * 2026-09-18: clang 18 stamps "408*"), and the llvm_gcda_* callback ABI
 * above has been stable since clang 11, the oldest kbuild accepts: any
 * clang unit this build can link, it can parse.
 */
...
/*
 * A live clang unit is what llvm_gcov_init() built above: this module's own
 * gcov_info and gcov_fn_info nodes around the consumer's counters and
 * filename. Free the nodes, never what they point at — gcov_info_free() is
 * for duplicates, whose counters and filename are copies.
 */
