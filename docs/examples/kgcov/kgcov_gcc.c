 ...
 * missed on the first pass. Vendored verbatim from Linux v6.8
 * kernel/gcov/base.c. The rest of that file (the global gcov_info list, the
 * module-unload notifier, gcov_event()) belongs to CONFIG_GCOV_KERNEL and is
 * intentionally not vendored: otto_kgcov keeps its own per-client object
 * list in kgcov.c instead, and gcov_event() is never called here.
 */
