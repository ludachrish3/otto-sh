...
management `hop` (see {ref}`lab-links` in {doc}`lab-config`). Links are the
**static underlay**: a `(host, interface)` pair on each end, the place where
`tc qdisc` actually attaches. Tunnels (see {doc}`tunnel`) are the **dynamic
overlay** — paths `otto tunnel add` lays *over* existing connectivity. A
tunnel is never impaired directly; impairing a link a tunnel happens to ride
affects that tunnel realistically, for free.
...
```{note}
The kernel's qdisc configuration IS the impairment state — there is no
separate ledger. `otto link list` reads `tc qdisc show` live, the same way
`otto tunnel list` reads live `socat` processes. Both directions of a link
are impaired by default; `--from` narrows to one.
```
...
declared — see {ref}`lab-links` in {doc}`lab-config`); both tab-complete
from the loaded lab.
...
endpoints — A→B on the interface of `pepper_seed` that **faces** `tomato_seed`,
B→A on the one facing `carrot_seed`. The facing interface is never declared:
it is auto-resolved at impair time by matching each endpoint's IP against
the middlebox's *live* interface subnets (`ip -o addr show`). If the
middlebox has no interface on an endpoint's subnet, resolution fails loud —
that failure doubles as the "is this host actually in the link's path"
check.

**Why `impair` only names the *where*, never the *how*:** the link-level
`impair` field says which host services the impairment; the host-level
`impairer` pin (see [Custom impairers](#custom-impairers)) separately says
which `LinkImpairer` implementation that host uses. Keeping the two
orthogonal — one field per concern — means adding a second impairer never
needs to touch the link's `impair` value, and vice versa.
...
- `[host_preferences]` in `.otto/settings.toml` can override the pin with an
  ...
  like `term`/`transfer` (see {ref}`host-preferences` in {doc}`lab-config`):
