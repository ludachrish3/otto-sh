...
| Field | Type | Description |
|-------|------|-------------|
| `os_type` | string | Profile selector.  Defaults to `"unix"`.  Resolves to a registered host class and optional defaults bundle — see {doc}`../hosts/os-profiles`. |
...
`os_type: "zephyr"` or `os_type: "embedded"`).  See {doc}`../hosts/embedded` for full
details.
...
See {doc}`../hosts/capabilities` for the Power Control section, runtime API
...
`otto tunnel add` (see {doc}`../network/tunnel`) rides to actually stand up traffic, and
that `otto link impair` (see {doc}`../network/link`) targets to impair it:
...
| `impair` | string | Optional in-path middlebox **host id** — a bare string, validated as a known host reference at lab load. When set, `otto link impair` (see {doc}`../network/link`) places both directions' netem on that middlebox's live-resolved facing interfaces instead of the link's own endpoints. `None` (the default) is endpoint-anchored impairment. |
