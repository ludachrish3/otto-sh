...
- **Optionally implement `get_reservation_windows()`** if your scheduler knows
  *when* bookings start and end (see
  [Reservation windows](#reservation-windows-optional-capability)).

## Reservation windows (optional capability)
...
({doc}`cli-reference`) caches its reservation answer for up to two minutes, and
...
[`assert_reservation_backend_conforms`](#verify-your-backend) checks these
