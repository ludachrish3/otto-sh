...
(see {doc}`../setup/lab-config` for the full host schema):
...
Term backends registered via `register_term_backend` complete the same way
built-ins do — a project-registered backend behaves identically to `ssh` or
`telnet` at the prompt:
...
selections are rejected at invocation time. See {doc}`../setup/lab-config` for
