...
{doc}`../setup/lab-config`; for the host classes that carry them, see {doc}`os-profiles`.
...
mid-connection. See {doc}`../setup/host-database` for the full `creds` field reference,
...
- {doc}`../setup/lab-config` — the `term` / `transfer` lab-data fields
- {doc}`../setup/host-database` — the `creds` field reference and login-proxy ownership
  consequences
- {doc}`../setup/editor-schemas` — `otto schema export` for editor autocompletion
