...
| `container` | a `[docker]` service, started by `otto docker up` (see {doc}`../../cli/docker/index`) | `chown` | `chown` | `chown` | `ignored` | no | bound at open | the parent host's own backend for the staging leg, then `docker cp` across the container boundary, `--concurrent` governing the staging leg | `user=` defaults to the service's declared user, and falls back to the image's own `USER` when neither is set. |
