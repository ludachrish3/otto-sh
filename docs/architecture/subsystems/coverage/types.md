...
three-tier workflow is covered in {doc}`../../../cli/cov/index`.
...
{doc}`../../../cli/cov/index` for the full valid/stale/aging/unverifiable
...
- **`thresholds`** — `{"high", "medium"}`, sourced from `[coverage.report]`
  (see {doc}`../../../cli/cov/index`); the render cutoffs above.
...
- **`overrides`** — the asserted-entry table, one `OverrideRecord` per
  entry loaded from the override file ({doc}`../../../cli/cov/index`):
