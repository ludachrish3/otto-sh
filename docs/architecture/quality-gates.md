...
| import budget | `profile` — gates module caps, snapshots, the denylist and the per-interpreter I/O goldens (including strace stat counters); never wall-clock, which fails for reasons outside the change | via `tests_hostless` | `tests` — enforced by `tests/unit/import_budget/` |
