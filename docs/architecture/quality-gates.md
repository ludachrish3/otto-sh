...
`.ast-grep/rules/` holds twenty rules — nineteen at `severity: error`,
...
| import budget | `profile` — no longer runs hyperfine: a wall-clock number fails for reasons outside the change, so it is manual (`make hyperfine` + `--hyperfine`) and the per-interpreter I/O goldens gate in its place | via `tests_hostless` | `tests` — enforced by `tests/unit/import_budget/` |
...
| declared harness bounds: each lane must *declare* its runaway guard rather than inherit a library default (vitest `testTimeout`/`hookTimeout`, Playwright's `expect.set_options` ceiling, pytest's `timeout`), each at or above a floor drawn from measured contention. A runaway guard discriminates nothing, so the gate does not care what the number is — only that a human wrote it, since a library default never appears in a diff. The vitest configs are discovered, not listed. **Not attempted:** finding a wall-clock bound written *into* an assertion (`assert elapsed < 5.0`). No gate looks for those; they are found by review and by the `serial_timing` marker's runtime backstop below | via `coverage*` | via `tests_hostless` | `tests` — enforced by `tests/unit/test_declared_harness_bounds.py` |
| probe-status honesty at the bed-probe helpers (`run_probe`/`probe_text`/`snapshot_host` raise on non-ok), plus AST scans banning the two consumer shapes found live: a factory unwrapping `.value` before the check can vet it (chaos lanes, where `run_probe` lives), and a `pkill`/`pgrep -f` pattern not built by `argv_pattern` — tree-wide over `tests/` since Wave 14, after the class regrew outside the lane scope (a self-killing cancel under a `suppress`, four `grep -v "$$"` probe mirrors). Stated blind spots: factories imported from other modules; patterns assembled outside literals/f-strings, including a pattern bound to a variable before the f-string (call `argv_pattern(...)` inline); exec-style list argv is out of scope (no wrapper shell to self-match) | via `coverage*` | via `tests_hostless` | `tests` — enforced by `tests/unit/test_bed_oracle_honesty.py` |
...
| private otto home: every test runs under a per-worker `OTTO_HOME` beneath pytest's basetemp (root conftest autouse), a test that unsets it to exercise the default pins `HOME` too, and the two out-of-process otto runners the tree owns (the e2e subprocess launcher, the docs capture script) set `OTTO_HOME` themselves — so nothing writes a workspace into, or walks, the developer's real `~/.otto` (8,974 leaked workspaces found there 2026-09-11, one per docs build) | via `coverage*` | via `tests_hostless` | `tests` — enforced by `tests/unit/test_otto_home_hermeticity.py` |
| skip policy: the chaos and embedded-coverage e2e lanes fail loud or pass, never skip (AST scan of both trees for `pytest.skip`/`importorskip`/`mark.skip`/`mark.skipif` — dotted with or without the `pytest.` prefix — plus the `from pytest import skip\|skipif\|importorskip` alias alley; the embedded-cov runner hard-fails on missing config or unbuilt artifact — the host-down rule extended to build/config absence). Stated blind spots, grepped zero at landing: `import pytest as pt`, `add_marker("skip")` strings, `unittest.SkipTest`, imperative `pytest.xfail`, out-of-tree parent conftests | via `coverage*` | via `tests_hostless` | `tests` — enforced by `tests/unit/test_no_skip_lanes.py` |
| BusyBox artifact tier: real prebuilt BusyBox binaries, fetched into `~/.cache/otto/busybox` from the `ci-assets-busybox-1` release mirror with busybox.net behind it — or from busybox.net alone in nightly's `busybox-upstream-drift` job, under `OTTO_BUSYBOX_SOURCE=upstream`, which is monitoring rather than a gate: drift cannot change a pinned byte, so it opens an issue instead of reddening a push (it sat on CI's `busybox` job until a 2026-08-26 outage reddened main for an unrelated change) (trust-on-first-use hash pins — upstream publishes no signatures) and executed, measuring the applet contracts `src/otto/host/userland.py` probes for. Excluded from **every** default lane, because the fixture RAISES rather than skips when its source is down and a cold cache costs a ~5 MB download: a fetch failure must never red the ordinary gates. Since the mirror answers first, only losing **both** hosts can red this job now — busybox.net alone going down reds nothing on push. That exclusion is not maintained by memory — `tests/unit/test_tier_marker_invariants.py` pins the catch-all clause (G8), the opt-in lane's existence (G8b), its invocation from a blocking, `report-failure`-reporting CI job (G8c), what each `-m` expression can actually *reach* (G8d), the tier's presence in `testpaths` (G8e), lanes that select by **path** with no `-m` at all (G8f, added after `unit-repeat` was found fetching on every push), and the directory stamp plus the explicit per-test markers (G9, G9b) | `busybox` (`busybox-cache` primes the cache only) | — | `busybox` — enforced by `tests/unit/test_tier_marker_invariants.py` |
...
| Python coverage floor (95.5, lines + branches) | `coverage`, `validate`, `all` | — | **not in CI** — local only; `release` runs a lower 92 floor via `nox -s tests_all` |
...
run at the 95.5 floor (a combined line+branch number, like every Python floor
here), plus the merged TypeScript floor — and no workflow runs it. CI gates
the *hostless* slice at 94.75 (`coverage-hostless` / the `tests` job),
because the full run needs lab VMs. A contributor reading only
`.github/workflows/ci.yml` would conclude the 95.5 floor is enforced on push.
...
prerequisite. The 95.5 floor is reached only through `make coverage`, and
...
So in practice the 95.5 floor is enforced by you, before you hand the work
over.
