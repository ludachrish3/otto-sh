...
| build-lane invariants: an `addopts` override keeps `-p no:tach`; every lane that excludes `serial_timing` re-appends it in a paired `-n0` leg (an exclusion without its leg makes the discriminators CI-invisible); and — the complement, since the leg check can only audit lanes that *spell* an exclusion — no lane running under xdist may select a `serial_timing` test at all, asked of pytest per lane (`--collect-only -m "<expr> and serial_timing"`) rather than inferred from decorators, because resource markers like `integration` and `browser` are applied by conftest hooks at collection and no source scan can see them. Make macros in a lane's `-m` are expanded first; an unexpanded one is reported, never handed to pytest | via `coverage*` | via `tests_hostless` | `tests` — enforced by `tests/unit/test_lane_invariants.py` |
