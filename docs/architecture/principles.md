...
a leaf — it must not import the app graph it describes. ({doc}`data-boundary`)
...
the seams cannot rot unnoticed. ({doc}`registries`)
...
test suite keeps startup cost from regressing. ({doc}`registries`,
{doc}`lifecycle`)
...
chatter can't hide failures. ({doc}`results-and-logging`)
...
become. ({doc}`results-and-logging`, {doc}`test-pipeline`)
