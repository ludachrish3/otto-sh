...
- I can *feel* the code sprawl and churn with the latest rounds of changes. I'd like to take a look at where code is churning the most, signs of refactoring scars, and ideas for refactoring to make changes and maintenance easier. Changes that should be simple, like updating the command timeout default, turned into hundreds of lines of change. Granted, real bugs were found and tests needed to be updated, but that's also a sign to me that otto's codebase is not gracefully aging. I'd like to brainstorm ideas to rectify this aging, but in technical fixes now, and policy changes in the future (possibly codified in AGENTS.md and in CONTRIBUTING.md) to mitigate this in the future. There's a lengthy commit history now, so hopefully that can help pinpoint where product and test areas are churning excessively. Code duplication and coupling are also things to look out for that might be causes of larger changes. As the codebase grows, it can be difficult to find a core mechanism to get something done, so a new approach is reinvented rather that reused or extended. In considering options, provide an effort estimate to each. DO NOT RULE OUT AN APPROACH STRICTLY DUE TO ITS COST. That decision is mine to make, and I'm more tolerant to cost to address tech debt. I want to keep otto development nimple and robust, so changing architecture and design is open in my mind before we have real users. Another area I'd like to consider for the refactor is the `otto host` subcommand. It currently uses the `@cli_exposed` decorator. It's feeling like the coupling between the host class and its CLI command are causing more harm than good. Would changing it back to a normal CLI command help with future changes? My original thought was that this mechanism would encourage feature parity between the CLI and the library, but the churn when updating the default command timeout taught me that this might not be the case. Command registration changed soon after using the `@cli_exposed` decorator on the host subcommand, so it could be that it should simply be registered like all other otto subcommands.
- A future feature I'd like to add to otto is code browsing (language server-style). I think that otto is fairly well set up for this due to its intimate knowledge of git history it has gained via all of the coverage report work. In the code view, it already has syntax highlighting, so the display of this is already well formed. At this time, I think that I'd just like to evaluate what otto currently has in terms of repo code display and refactoring it to aid in sharing parts with code browsing. I think both coverage reports and code browsing would benefit from a search box in the app bar activated via `/` to look up symbol names. To be clear, I DO NOT want to implement the code browser feature at this time. I just want to lay the groundwork to make that work easier and set it up for success. The code browser is also eventually meant to be a code walkthrough tool with call stacks, task highlighting, scenario recording (via clicking on symbols), and more. Again, THIS IS JUST DIRECTIONAL INFO. DO NOT IMPLEMENT ANYTHING YET FOR CODE BROWSING. My intent is to give you enough info to properly decompose what already exists to set future changes up for success.
