...
- Login proxy is likely up next in terms of priorities. Some users need to login via another account first and take unusual steps to do tasks. Think of a mysql user - it typically has no login capability, so you need to login as a different user, switch users in some way, and *then* you can enter a mySQL shell. A tricky part of this is *where* the logic to proxy from one user to another should live. It's really owned by the host and not a project, so it doesn't necessarily have an obvious place to live. Coupling it to a project doesn't make sense. Allowing separate libraries to provide registered actions might be a good path because otto projects already init and import other libraries on startup. Passwording blinding is NOT A GOAL. The current use case is for hosts that are in an air-gapped network and accessible team-wide, so keeping these passwords secret are not a priority at this time. A couple goals from this:
  - "Proxying' the login user is an important and novel path compared to what's possible today. I'd like for a username to be able to log in as another user first and then take some number of arbitrary steps (specified via a loginProxy registry key) to become the desired user. This is a big tricky because the creds dict only holds username as a key and and the password as the value. This might need to be changed to support a more dynamic structure.
  - Application shells (sometimes called REPLs) is another paradigm that otto does not currently handle gracefully. The class name could be something like `AppShell` or `ApplicationShell` or `ReplShell`. It would be a series of `send()`s and `expect()`s, which is doable but not at all ergonomic. It would be very convenient to define a shell, like for MySQL or even the python interpreter, to take away the cruft of managing an application shell.
    - I believe this shell would need a regex prompt. I typically prefer to avoid regex prompts due to their likelihood to accidentally not match, but I believe this might be needed to generally work in all cases. The command frame concept could be extended in this scenario, but I think it might be more trouble than it's worth and not all shells can support a command frame. I'm leaning toward an explicit, required prompt regex to notice when commands complete.
    - I'd like app shells to also allow for convenient command issuing and regex parsing of output into an object. For example, if a mySQL application is defined, it could be that `MySql.select()` is a method, and it returns a `SqlTable` object that parses the table printed out by the mySQL REPL. `pydantic` seems like a good fit for the comamnd parsing into an object to ensure type conversions and validation. Regex parsing with named groups will provide a dict of names and stringified values, which is not the best to deal with. This part hopefully fits in naturally with the `Result` types, where the value would be the object after being converted from the regex dict.
