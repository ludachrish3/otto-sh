...
- Login proxy is likely up next in terms of priorities. Some users need to login via another account first and take unusual steps to do tasks. Think of a mysql user - it typically has no login capability, so you need to login as a different user, switch users in some way, and *then* you can enter a mySQL shell. a couple goals from this:
