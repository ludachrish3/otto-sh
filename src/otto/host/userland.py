...
``applet_<name>``, one per entry in :data:`PROBED_APPLETS`
    ...
    presence), and ``nc``'s presence is an answer about the NAME ``nc`` while
    the backend execs whatever ``NcOptions.exec_name`` says -- see
...
the frame registry; the other five are safe to consume today, and all
...
``stat``'s ``-c %s`` vs ``--format=%s``, and ``wc``'s ``-c <`` vs ``-c FILE``.
...
# 3 timeout + 2 base64 + 2 stat + 1 checksum + 1 dialect + 1 applet batch),
# so a host that swallows every one of them costs 12 x _PROBE_TIMEOUT_S = 120s
# unbounded. The
...
# applet does not move this number; adding or removing a CAPABILITY does, and
# retiring ``nc_dash_n`` is what moved it back from fourteen. resolve() holds its
...
# 40.0 keeps all 99 tests green; 41.0 already reds
...
# one test, two distinct ways to fail it, not two tests); 101.0 adds a second
# test failing; 111.0 adds a third. Neither prior number was ever guarded by
# an assertion that reads it, which is exactly how both survived being wrong
# until someone happened to measure them.
#
# RE-MEASURED 2026-08-25, when `nc_dash_n` was retired: the last two moved
# (they were 111.0 and 121.0 while it existed) and the first two did not, and
# the test count fell from 102 to 99 with the three field-parametrized rows
# that capability supplied. That is the whole lesson of this comment in one
# change -- the boundaries are properties of the probe TREE, so a capability
# added or removed anywhere in it shifts the ones that count commands and
# leaves the ones that count budget alone.
#
...
#    directly from the failing assertion's own captured call list), never 12,
#    no matter how much budget the caller affords. Once the budget affords 11
#    or more, the test's own `affordable = ceil(budget / probe)` prediction
#    exceeds what the algorithm can structurally send, and
#    `len(runner.calls) == affordable` reds on a probe-tree ceiling that
#    has nothing to do with the time budget (101.0).
# 3. test_every_logger_call_site_is_exercised_at_debug is the third red, at
#    111.0, and it breaks for a reason that has nothing to do with either
...
#    budget. Its third scenario scripts a device that ANSWERS
#    `command -v timeout` and swallows everything after it, so unlike (2)
#    nothing short-circuits and all TWELVE commands are reachable -- which is
#    why this boundary sits one `affordable` step past (2)'s rather than on
#    it. Once the budget affords twelve, nothing is ever skipped, that line is
#    never emitted, and the log-site coverage guard reds reporting a call site
#    "never exercised". BEWARE when chasing this one: the failure message is
#    about logging and reads like a logging defect; the actual cause is this
#    constant. Check the budget row of
#    test_resolution_stops_once_the_whole_budget_is_spent (and mechanism 2
#    above) before believing the message.
#
# All three are properties of their SCRIPTS, not of this constant in general
# -- a differently-scripted
...
-- ``nc`` from the ``nc-transfer`` record, which has since been closed and
whose name stays here because the list is what :meth:`Userland.has_applet`
answers for, not a mirror of the registry. Which piece of work reads which
name, and how far presence actually gets each one:
...
``nc``
    the ``nc`` transfer backend, and the one entry whose answer is not about
    the binary otto will actually exec.
    :attr:`~otto.host.options.NcOptions.exec_name` chooses that, and it may
    name a binary (``ncat``, ``netcat``) this closed list does not carry at
    all, so ``present`` says nothing about WHICH netcat answered. Only the
    ``absent`` direction is a conclusion, and only about the name ``nc``
    itself: nothing to run is nothing to run. There is no second,
    option-shaped question beside this one any more -- the backend emits one
    spelling every measured netcat accepts, which is what closed the
    ``nc-transfer`` record
    (``docs/superpowers/specs/2026-08-25-nc-universal-spelling-design.md``).
    This entry has no consumer.

WHAT IS DELIBERATELY NOT HERE. ``sftp-transfer`` is a blocked surface too
...
# ``stat_size``
#     ``stat``. ``transfer/nc.py`` has always sized remote files with `stat`
#     (three call sites -- `-c %s` for PUT's verify, `-L -c '%s %F'` at both
#     GET prefetches), so this is the status quo; ``absent`` would be a
#     capability regression rather than a neutral guess.
# ``checksum``
...
class Userland:
    ...
    async def resolve(self) -> None:
        ...
        **WHAT A CALLER PAYS.** Every capability, not the one it came
        ...
        settled. Twelve rather than one per capability because several
    ...
    async def _resolve_once(self) -> None:
        ...
        #
        # LAST, deliberately. The six above keep their exact order, their
        ...
        # incumbent. A capability added later belongs after them for the same
        # reason -- the newest one is what a short budget should lose.
        #
...
# :func:`otto.host.transfer.scp.refuse_if_scp_is_absent`, not here -- and they
# do not pay the same price, which is why each argues its own. Only the scp one
# ADDS a resolution to a path that awaited none before.
#
...
class Gap:
    ...
    **Name the code by what it EMITS, not by ``file.py:<line>``.** This string
    ...
    names a test resolves. NO RECORD BELOW CITES ONE ANY MORE: the last two
    went out with ``shutdown-command``'s rewrite and with the since-closed
    ``nc-transfer`` record's. Keep it that way -- the sites are in ``paths``,
    where something resolves them.
    """
