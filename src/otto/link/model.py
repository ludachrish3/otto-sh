...
class Provenance(enum.Enum):
    ...
    DYNAMIC = "dynamic"
    """Realized by otto tunnels; reserved for topology overlays."""
...
class LinkEndpoint:
    ...
    port: int | None = None
    """Reserved/observed-port metadata for this end."""
...
def make_link_id(a: LinkEndpoint, b: LinkEndpoint, protocol: str) -> str:
    ...
    STABILITY CONTRACT — changing this algorithm invalidates every recorded
    static route id across otto versions:

    - endpoints are sorted by ``(host, interface or "")`` so a<->b == b<->a;
    - a ``None`` interface falls back to ``''`` in the canonical string;
    - *protocol* is lowercased, so a route declared ``"udp"`` and one declared
      ``"UDP"`` reconcile to the same id;
    - **ports and ips are excluded** — the id names the route, not any one
      observation of it;
...
class Link:
    ...
    id: str = ""
    """Static id, auto-computed when empty via ``make_static_link_id``
