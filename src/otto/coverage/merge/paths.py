"""Path remapper: normalise source paths across build host, remote hosts, and gcda metadata.
...
async def discover_path_mappings(
    ...
    """
    result = await localhost.exec(f"grep '^SF:' {info_path}")
...
async def discover_from_gcno(
    ...
    result = await localhost.exec(f"find {gcno_dir} -name '*.gcno' -type f | head -20")
    if result.retcode != 0 or not result.value.strip():
    ...
    for gcno in gcno_files[:5]:  # sample a few files
        strings_result = await localhost.exec(f"strings {gcno}")
