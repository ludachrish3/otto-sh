...
therefore means "one of otto's 56 NAMED failures", not "anything otto
raised".
...
* ``except (ValueError, RuntimeError)`` covers 337 of the 387 raise sites,
  and 38 of the 56 named classes. Of the other 18, twelve are rooted at plain
  ``Exception`` (the bootstrap, project-activation, lab-context,
  lab-repository, inventory, creds and reservation errors) and six sit under
  ``OSError`` (``AppShellTimeoutError``, ``LoginProxyError``,
  ``NcPortSharedError``, ``RetryAttemptTimeoutError``, ``SessionSetupError``,
  ``WaitTimeoutError``) — 38 + 12 + 6 = 56, so the split accounts for every
  named class.
