"""CSS color vocabulary and validation (stdlib-only model helper).

Color values come from user config (e.g. ``[coverage.tiers.<name>] color``
in settings.toml) and may be a CSS named color or a ``#RRGGBB`` hex code.
Validation happens at settings load; consumers use values verbatim.
"""
