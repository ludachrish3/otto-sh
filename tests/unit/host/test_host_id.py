...
def test_make_host_id_multiword_element_slugs():
    assert make_host_id("Lab X Server", None, None) == "lab-x-server"
    assert make_host_id("Lab X Server", "Line Card", 2) == "lab-x-server_line-card2"
...
def test_make_host_id_only_underscore_is_board_delimiter():
    ...
    hid = make_host_id("edge node", "line card", 3)
    assert hid == "edge-node_line-card3"
