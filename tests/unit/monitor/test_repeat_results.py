...
class TestStoreRepeatResult:
    ...
    def test_stores_result_on_success(self, host):
        ...
        task = self._make_task(result=RunResult(status=Status.Success, statuses=results))
    ...
    def test_respects_maxlen(self, host):
        ...
        for _ in range(5):
            task = self._make_task(result=RunResult(status=Status.Success, statuses=single))
            host._repeater._store_result("myjob", task, None)
