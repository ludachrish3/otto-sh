...
The frontend re-declares, by hand and with no compiler in between, three
things this package owns: the keys of the ticket payloads
(``web/src/covapp/types.ts``), the sentinel ticket ids (as TS string
literals), and the ``window`` callback names the emitted chunk scripts
invoke. ``tests/_fixtures/covapp_ticket_contract.json`` is the shared
