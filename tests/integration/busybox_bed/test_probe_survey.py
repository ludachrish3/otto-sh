...
construction. That gives the survey three live facts to prove: the netstat
arm parses on every version, the owner column is read (root, so it is
populated), and the permanently dead, undeclared ssh is a drift
true-negative, not a false supported.
"""
...
@pytest.mark.asyncio
async def test_the_survey_reports_telnet_supported_and_ssh_as_the_drift_true_negative(
    hop_on_this_loop, guest
):
    """The guest's cred is root, so the inventory runs elevated: no owners-incomplete notice.

    ssh and ftp are dead by construction and the guest's menu names neither,
    so they are the drift true-negatives. Both are declared candidates, so
    each is dialed from the guest's own vantage — the ``test1`` hop — before
    any login is tried; the dial is refused and no login is attempted.

    The row that survives is the INVENTORY's, not the dial's: both say
    ``closed``, and the inventory tier outranks the dial tier. That is the
    assertion, and it is the whole point — the tier is the evidence that no
    login ran. A login-tier row would outrank both, so the only way this row
    can read ``inventory`` is if the pre-dial stopped the login. Before it
    did, this port read ``login-failed`` from a hop-forwarded connect whose
    reset the classifier took for a refused password: an ANSWER, which drifts
    and pins, on a port with no service at all.
    """
    host, version = guest
    ...
    for dead in ("ssh", "ftp"):
        row = next(v for v in s.verdicts if v.protocol == dead)
        ...
        assert dead not in s.supported, (version, s.supported)
    ...
    # above still green.
    dial = await dial_for(host, timeout=DIAL_TIMEOUT_S)(_DEAD_SSH_PORT)
