...
# carrot impaired forever, not the expected schedule -- worst case observed
...
def test_reboot_x_tunnel_survivors_reaped(tmp_path):
    ...
    try:
        ...
        msg = f"tomato reboot --wait recovered in {elapsed:.1f}s (tunnel scenario)"
        print(msg)  # noqa: T201 -- recorded down/up/recovery data point
...
def test_reboot_x_link_repair_idempotent(tmp_path):
    ...
    means: netem gone on tomato, still present on carrot, until `repair --all`.
    """
    ...
    try:
        ...
        elapsed = time.monotonic() - started
        # Recorded down/up/recovery data point; captured on failure/verbose.
        msg = f"tomato reboot --wait recovered in {elapsed:.1f}s (link scenario)"
        print(msg)  # noqa: T201 -- recorded down/up/recovery data point
        assert rc == 0, rp.stderr_text()
        ...
        assert_eth2_netem_free("repair --all against the half-clean (tomato-already-clean) state")
    finally:
